    ...
    for cnt in range(test_time):
        with torch.no_grad():
            for inputs, labels in loaders.test:
                ...
                predM = models.rr(s1n.detach(), s2n.detach(), s3n.detach(), s4.detach())
                if cnt == 0:
...
def train_model(models, loaders, optims, config):
    ...
    for t in range(config.nepoch):
        ...
        for inputs, labels in tqdm(loaders.train):
            ...
            predM = models.rr(s1n, s2n, s3n, s4)
        ...
        message = f'EP:{t:03d} | MaxSRCC: {srcc_val_t : .3f} | MaxPLCC: {plcc_val_t : .3f} |Extra Info {config.log}\n'
...
def getHyperParams(pct, sd):
    myconfigs = {
        ...
        'batch_size': 12,
        'data_lens': 10073,
        'roots': r'/root/IQA_Dataset/KONIQ',
        'type': 'LCIQA',
        'log': 'Test02_IMG_768x512->384x612_Repeat10_WithSubLayerOptim'
    }
