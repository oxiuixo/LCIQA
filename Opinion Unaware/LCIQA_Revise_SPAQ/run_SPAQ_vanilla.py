...
def test_model(models, loaders, config, cnt):
    ...
    srcc_valM, _ = stats.spearmanr(predM_vals.squeeze(), mos_vals.squeeze())
    plcc_valM, _ = stats.pearsonr(predM_vals.squeeze(), mos_vals.squeeze())

    return srcc_valM, plcc_valM

def train_model(models, loaders, optims, config):
    torch.cuda.empty_cache()
    models.bb.train(True)
    ...
    for t in range(config.nepoch):
        ...
        srcc_valM, plcc_valM = test_model(models, loaders, config, t)
...
def main(pcfg):
    ...
    paramQ = [
        ...
        {'params': mres.parameters(), 'lr': myconfig.lr},
    ]
